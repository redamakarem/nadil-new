<!doctype html>
<html lang="{{ app()->getLocale() }}" @role('user')
...
<body class="">
    <main class="bg-black">
        <div id="mobile-wrapper" class="relative min-h-screen bg-black" x-data="{ isOpen: false }">
            <div class="sidebar flex-col fixed z-50 bg-white dark:bg-black inset-y-0 {{ app()->getLocale() == 'en' ? 'left-0' : 'right-0' }} z-10 w-2/3 max-w-md transform transition duration-600"
                ...
                <a href="{{ route('site.home') }}"
                    class=" block uppercase text-black dark:text-white py-3 px-8 {{ request()->is('/') ? 'menu-active' : '' }}">{{ __('nadil.menu.discover') }}</a>

                @role('user')
                    <a href="{{ route('user.profile.show') }}"
                        class=" block uppercase text-black dark:text-white py-3 px-8 {{ request()->is('profile*') ? 'menu-active' : '' }}">{{ __('nadil.menu.profile') }}</a>
                    <a href="{{ route('user.history.show') }}"
                        class=" block uppercase text-black dark:text-white py-3 px-8 {{ request()->is('history') ? 'menu-active' : '' }}">{{ __('Reservations') }}</a>
                    ...
                    <form action="{{ route('logout') }}" method="POST"
                        ...
                        @csrf
                        <a href="#"
                            class=" block uppercase text-black dark:text-white py-3 px-8">{{ __('nadil.menu.logout') }}</a>
                    </form>
                @endrole
                @guest
                    <a href="{{ route('login') }}"
                        class=" block uppercase text-black dark:text-white py-3 px-8 {{ request()->is('login*') ? 'menu-active' : '' }}">{{ __('nadil.menu.login') }}</a>
                @endguest
                @guest
                    <a href="{{ route('register') }}"
                        class=" block uppercase text-black dark:text-white py-3 px-8 {{ request()->is('register*') ? 'menu-active' : '' }}">{{ __('nadil.menu.register') }}</a>
                @endguest
                <a href="{{ route('site.about') }}"
                    class=" block uppercase text-black dark:text-white py-3 px-8 {{ request()->is('about*') ? 'menu-active' : '' }}">{{ __('nadil.menu.businesses') }}</a>
                <div>
            ...
            @if (!empty($current_restaurant))
                ...
                @else
                    ...
                    ">
            @endif
            <div class="flex justify-end px-8 pt-24 h-20">
        ...
        <div
            class="content flex-col items-center bg-nadilBg-100 rounded-[60px] min-h-[70vh] shadow-lg relative -top-16">
            @yield('content')

        </div>
        <div class="bg-black text-white flex justify-between px-12 py-8">
            <div class="flex flex-col space-y-4 items-center w-full">
                <div class="uppercase text-xs font-lato rtl:font-ahlan"><a
                        href="">{{ __('nadil.footer.terms') }}</a></div>
                <div class="uppercase text-xs font-lato rtl:font-ahlan"><a
                        href="">{{ __('nadil.footer.cancellation') }}</a></div>
                <div class="uppercase text-xs font-lato rtl:font-ahlan"><a
                        href="">{{ __('nadil.footer.faq') }}</a></div>
                <div class="uppercase text-xs font-lato rtl:font-ahlan"><a
                        href="">{{ __('nadil.footer.contact') }}</a></div>
            </div>
        </div>
        </div>
