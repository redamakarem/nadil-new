<div class="w-full">
    ...
    <div class="flex flex-col items-center w-full ">
            ...
            <div class="w-1/2 ">
                ...
                @if ($selected_date && $seats)
                    <div
                        ...
                        @if ($slots)
                            @foreach (array_chunk($slots, 3, true) as $chunk)
                                <div class="flex justify-center">
                                    @foreach ($chunk as $key => $timeSlot)
                                        <div role="group">

                                            <input type="radio" id="t-{{ $key }}" class="rb-timeslot chk-btn"
                                                {{ $this->slot_bookable($timeSlot) ? '' : 'disabled' }}
                                                wire:model='selected_time' value='{{ $timeSlot }}'>
                                            <label for="t-{{ $key }}"
                                                class="mb-4 inline-block px-6 py-4 bg-nadilBtn-100 rounded-[19px] disabled:text-gray-400 hover:bg-grey-500 focus:bg-black focus:text-white checked:bg-black checked:text-white focus:outline-none focus:ring-0 active:bg-black active:text-white transition duration-150 ease-in-out">{{ \Carbon\Carbon::parse($timeSlot)->translatedFormat('h:i A') }}
                                            </label>
                ...
                @else
                <div class="h-[296px] flex flexcol justify-center items-center px-8">
                    <div>Please select date and number of guests</div>
                </div>
                @endif
        ...
        <div class="flex my-8 flex-col">
            <div class="font-lato rtl:font-ahlan uppercase text-md text-center">
                {{ __('nadil.booking.how_many_seats') }}
            </div>
